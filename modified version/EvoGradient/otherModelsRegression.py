...
class Transformer(nn.Module):
    def __init__(self,batch_size=128,embedding_size=20,num_tokens=100,num_filters=100,filter_sizes=(2, 3, 4),num_classes=1,num_heads=4):
        ...
        self.batch_size      = batch_size
        self.embedding_size  = embedding_size
        self.num_tokens      = num_tokens          # original one-hot length
        self.num_filters     = num_filters
        self.filter_sizes    = filter_sizes
        self.num_classes     = num_classes
        self.num_heads       = num_heads

        # ----- CNN part (produces 32-token feature map) -----
        ...
        # 3 filters → 3 * 100 = 300 features → reshape to 32 × hidden1
        self.hidden1 = embedding_size                # 20
        self.cnn_out_len = 32                        # <-- fixed after pooling
        self.fc_cnn = nn.Linear(len(filter_sizes) * num_filters,
                                self.cnn_out_len * self.hidden1)

        # ----- Transformer encoder part -----
        self.num_encoder   = 2
        self.hidden2       = 128
        self.dropout_rate  = 0.3
        self.pad_size      = self.cnn_out_len        # <-- 32, NOT 100

        self.device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")

        # **NO positional encoding** – the CNN output is already a fixed-length map
        # self.postion_embedding = Positional_Encoding(...)

        self.encoder = Encoder(self.hidden1,
                               self.num_heads,
                               self.hidden2,
                               self.dropout_rate)

        self.encoders = nn.ModuleList([
            copy.deepcopy(self.encoder) for _ in range(self.num_encoder)
        ])

        # ----- final classifier -----
        self.new_fc1 = nn.Linear(self.pad_size * self.hidden1, self.num_classes)

        # (optional) if you ever feed raw one-hot directly:
...
class Positional_Encoding(nn.Module):
    def __init__(self, embed, pad_size, dropout, device):
        ...
        # Create pe directly on the correct device (MPS/CPU)
        ...
        div_term = torch.exp(torch.arange(0, embed, 2, device=self.device).float() * (-math.log(10000.0) / embed))
        
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        
        self.pe = nn.Parameter(pe, requires_grad=False)  # Already on device
