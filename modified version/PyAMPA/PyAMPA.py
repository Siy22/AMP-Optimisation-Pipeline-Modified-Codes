...
class DataFrameViewer(tk.Toplevel):
    ...
    def mutagenesis(self):
        ...
        # Create a comprehensive DataFrame for all mutations
        mutations_data = []
        sequence_length = len(sequence)
        amino_acids_list = list(amino_acids)  # 'ACDEFGHIKLMNPQRSTVWY'
        for pos in range(sequence_length):
            for aa_idx, new_aa in enumerate(amino_acids_list):
                idx = pos * len(amino_acids_list) + aa_idx
                mutated_seq = sequence[:pos] + new_aa + sequence[pos + 1:]
                row = {
                    'Position': pos + 1,  # 1-based for readability
                    'Original_AA': sequence[pos],
                    'Mutated_AA': new_aa,
                    'Mutated_Sequence': mutated_seq,
                    'AMP_Probability': probabilities_amp[idx],
                    'Hemolytic_Probability': probabilities_hemo[idx],
                    'Half_Life': half_lives[idx],
                    'Fitness_Score': fitness_scores[idx]
                }
                mutations_data.append(row)

        # Convert to DataFrame
        mutations_df = pd.DataFrame(mutations_data)

        # Round numerical columns for readability
        numerical_cols = ['AMP_Probability', 'Hemolytic_Probability', 'Half_Life', 'Fitness_Score']
        for col in numerical_cols:
            mutations_df[col] = mutations_df[col].apply(lambda x: round_to_sig_figs(x, 4))

        # Save to CSV in the output directory
        output_file = f"{self.master.output_directory}/mutagenesis_results_{sequence[:10]}.csv"
        mutations_df.to_csv(output_file, index=False)
        print(f"Mutagenesis results saved to: {output_file}")

        # Optional: Open the CSV automatically (macOS)
        import subprocess
    ...
    def optimization(self):
        ...
        for generation in range(NUM_GENERATIONS):
            ...
            # Selection
            # Selection - FIX FOR NEGATIVE FITNESS SCORES
            min_fitness = min(fitness_scores)
            if min_fitness < 0:
                # Shift all scores to be positive
                shifted_scores = [f - min_fitness + 1 for f in fitness_scores]
            else:
                shifted_scores = fitness_scores
            
            # Ensure we have positive weights
            if sum(shifted_scores) <= 0:
                # If all scores are still zero/negative, use uniform selection
                selected_individuals = random.choices(population, k=POPULATION_SIZE)
            else:
                selected_individuals = random.choices(population, weights=shifted_scores, k=POPULATION_SIZE)
...
class Application(tk.Tk):
    ...
    def run_pyampa(self):
        # Run pyampa.py
        result = subprocess.run(["python", "pyampa.py", "-i", self.input_file, "-o", self.output_directory], 
                          capture_output=True, text=True)
    
        # Check if pyampa.py found any results
        results_file = f"{self.output_directory}/results.csv"
    
        if os.path.exists(results_file):
            df = pd.read_csv(results_file)
        
            # If no peptides were found, create one with the input sequence
            if len(df) == 0:
                print("No peptides found by screening. Adding input sequences directly...")
            
                # Read the input FASTA file
                from Bio import SeqIO
                sequences = list(SeqIO.parse(self.input_file, 'fasta'))
            
                # Add all sequences from input file to results
                data = []
                torrent_index = {
                    'R': 0.106, 'K': 0.111, 'C': 0.165, 'W': 0.172, 'Y': 0.185,
                    'I': 0.198, 'V': 0.200, 'H': 0.202, 'N': 0.240, 'T': 0.242,
                    'F': 0.246, 'L': 0.246, 'Q': 0.248, 'G': 0.265, 'M': 0.265,
                    'S': 0.281, 'A': 0.307, 'P': 0.327, 'E': 0.449, 'D': 0.479
                }
                ...
                for seq_record in sequences:
                    ...
                    # Calculate average torrent index
                    try:
                        avg_torrent = sum(torrent_index[aa] for aa in sequence) / len(sequence)
                    except KeyError:
                        avg_torrent = 0.200  # default for non-standard amino acids
                
                    data.append({
                        'Name': name,
                        'Peptide': sequence,
                        'Average Torrent Index': avg_torrent
                    })
            
                # Save to results.csv
                df = pd.DataFrame(data)
                df.to_csv(results_file, index=False)
                print(f"Added {len(data)} sequences to results.csv")
        else:
            # If results.csv wasn't created at all, show error
            messagebox.showerror("Error", "Failed to create results file. Please check your input.")
            self.popup.destroy()
            return
