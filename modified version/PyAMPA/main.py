...
class Application(tk.Tk):
    ...
    def run_pyampa(self):
        ...
        # Check if inputs are short known peptides (≤50 aa) or long proteome sequences
        all_short = all(len(str(seq.seq)) <= 50 for seq in sequences)

        if all_short:
            # Bypass screening — use full sequences directly (e.g. TTC, Magainin 2)
            data = []
            ...
            # Run pyampa.py screening for proteome/long sequences
            subprocess.run(["python", "pyampa.py", "-i", self.input_file, "-o", self.output_directory],
